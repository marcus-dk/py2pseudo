    ...
    for line in to_pseudo:
                        ...
                        try:
                            ...
                            for i in range(count,-1,-1):
                                ...
                                for j in range(j_use,-1,-1):
                                    ...
                                    if line[i][j] in stop_chars or line[i][j] in "(){}[]":
                                        raise Exception
                        except:
                            n_p_c = False
                            ...
                            if start_index2!= -1:
                                ...
                                for i in range(begin,len(line)):
                                    ...
                                    if line[i] in "({[]})":
                                        ...
                                        if line[i:i+5] == "RANGE":
                                            ...
                                            if i+5!=len(line):
                                                out = True
                                            elif ((out ==-1) or line[i+5] not in stop_chars) and (line[i-1] not in stop_chars):
                                                range_found = True
                                                coords = i+5
                                            break
                                ...
                                if range_found:
                                    ...
                                    for i in range(coords,len(line)):
                                        ...
                                        elif stack_of_exceptions == ["("]:
                                            if line[i]==",":
                                                separate_by.append(i)
                                            look_B = True

                                        elif look_B and stack_of_exceptions == []:
                                            index_B = i
                                            break
        ...
        def receiver(line,replace_list,add_print_n_receive,input_name_counter,og,self_r,tabs):
            ...
            while i <len(line):
                ...
                elif line[i:i+6] == "RECEIVE":

                    if (i == 0 or line[i-1] not in stop_chars) and (i+6 == len(line) or line[i+6] not in stop_chars):
                        temp_stack = []
                        bracket = True
                        start_stack = False

                        for j in range(i+6,len(line)):
                    ...
                    if not(((line[index-1] in equal_chars or line[index-1] in front_chars) and index !=0) or line[index+2] in equal_chars):
                        ...
                        if not(line[index-1] in equal_chars or line[index-1] in front_chars or line[index+2] in equal_chars):
                            ...
                            while i <len(line):
                                ...
                                if (line[i:i+7] == 'RECEIVE(' or bracket) and single:

                                    if (line[i:i+7] == 'RECEIVE(' and bracket!=True):
                                        start = i+7
                                        i+=6
